...
## 💡 5. Quyết định thiết kế: Mô phỏng Open-Drain (Yêu cầu 4.3)

Dựa trên yêu cầu mô phỏng cấu trúc ngõ ra Open-Drain của IC TPS3431, dự án Dashboard giả lập này đang áp dụng giải pháp tương đương **Cách B (Push-pull giữ nguyên quy ước Active-Low)**:
- **Đối với WDO:** Vì môi trường Web/React không có khái niệm trở kháng cao (High-Z) vật lý như phần cứng thực, UI sử dụng biến logic `wdo`. Khi bình thường (High-Z thực tế), luồng code xuất `1` (Đèn D3 tắt). Khi có lỗi, luồng code xuất `0` (Đèn D3 sáng Đỏ). 
- **Đối với ENOUT:** Xuất mức `1` khi báo Ready và `0` khi vô hiệu hoá.
- **Lý do chọn giải pháp này:** Giải pháp này đảm bảo tính trực quan 100% về mặt thời gian (timing) và bảng chân lý (truth table) trên giao diện giám sát so với một mạch RTL thực thụ mà không cần phức tạp hóa UI engine.
